...
struct MapScreen: View {
    ...
    var body: some View {
        ...
        .confirmationDialog("Đặt vị trí thủ công?",
                            isPresented: overrideDialogBinding,
                            titleVisibility: .visible) {
            Button("Đặt vị trí tại đây", role: .destructive) { applyPendingOverride() }
            Button(L10n.cancel, role: .cancel) { pendingOverride = nil }
        } message: {
            Text("\(coordinator.activeSource?.displayName ?? "Một nguồn khác") đang điều khiển GPS. Đặt vị trí thủ công sẽ giành quyền và dừng nguồn đó.")
        }
    }
