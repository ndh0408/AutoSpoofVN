...
///
/// Mỗi thuộc tính tra thẳng vào `Localizable.strings` qua `LocalizationManager`, nên
/// người dùng đổi ngôn ngữ trong Cài đặt là đổi ngay, không cần đổi ngôn ngữ máy.
/// Trước đây enum này chỉ là hằng chuỗi tiếng Việt cứng — hai thư mục `en.lproj` và
/// `vi.lproj` vẫn nằm đó nhưng **không file Swift nào đọc tới**.
enum L10n {
    // Mô phỏng
    static var simulationRunning: String { L("simulation.running") }
    static var simulationPaused: String { L("simulation.paused") }
    static var simulationIdle: String { L("simulation.idle") }

    // Thiết bị
    static var deviceConnected: String { L("device.connected") }
    static var deviceDisconnected: String { L("device.disconnected") }

    // Tuyến đường
    static var routeDistance: String { L("route.distance") }
    static var routeETA: String { L("route.eta") }
    static var routeSource: String { L("route.source") }

    // Hành động
    static var start: String { L("action.start") }
    static var stop: String { L("action.stop") }
    static var pause: String { L("action.pause") }
    static var resume: String { L("action.resume") }
    static var retry: String { L("action.retry") }
    static var diagnostics: String { L("action.diagnostics") }
    static var cancel: String { L("action.cancel") }
    static var close: String { L("action.close") }
    static var done: String { L("action.done") }
    static var save: String { L("action.save") }
    static var delete: String { L("action.delete") }

    // Trạng thái rỗng
    static var noRoutes: String { L("empty.routes") }
    static var noRoutesMessage: String { L("empty.routes.message") }
    static var createRoute: String { L("empty.routes.action") }
    static var noDevice: String { L("empty.device") }
    static var noDeviceMessage: String { L("empty.device.message") }

    // Tab
    static var tabMap: String { L("tab.map") }
    static var tabRoutes: String { L("tab.routes") }
    static var tabFlight: String { L("tab.flight") }
    static var tabSettings: String { L("tab.settings") }
}
