...
struct ManualCoordinateSheet: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    LabeledContent(L("common.latitude")) {
                    ...
                    LabeledContent(L("common.longitude")) {
                ...
                Section {
                    Button {
                        ...
                        Label(L("map.manual.paste"), systemImage: "doc.on.clipboard")
                    }
                    Button {
                        ...
                        Label(L("map.manual.use_current"), systemImage: "location.circle")
                    }
                }
                ...
                if coordinator.state.isActive, coordinator.activeSource != .manual {
                    Section {
                        Label {
                            Text(L("map.manual.takeover_warning",
                                   coordinator.activeSource?.displayName ?? L("map.manual.other_source")))
                        } icon: {
            ...
            .navigationTitle(L("map.manual.title"))
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button(L("map.manual.apply")) { apply() }
                        .fontWeight(.semibold)
                        .disabled(parsed == nil)
                }
            }
    ...
    private var validationError: String? {
        ...
        guard let lat = Double(latTrimmed) else { return L("map.manual.error.lat_nan") }
        guard let lon = Double(lonTrimmed) else { return L("map.manual.error.lon_nan") }
        if !(-90...90).contains(lat) { return L("map.manual.error.lat_range") }
        if !(-180...180).contains(lon) { return L("map.manual.error.lon_range") }
        return nil
    }
